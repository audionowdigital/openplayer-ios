...
- (void)initNetworkCommunication:(NSString*)urlStr1 {
    NSString *urlStr = @"http://www.markosoft.ro/opus/02_Archangel.opus";
    if (![urlStr isEqualToString:@""]) {
        ...
        } else NSLog(@"%@ host, port: %d", [website host], [website port]);
...
- (void)stream:(NSStream *)theStream handleEvent:(NSStreamEvent)streamEvent {
	...
	switch (streamEvent) {
		...
		case NSStreamEventOpenCompleted:
			NSLog(@"Stream opened");
            /*char buf[100];
            int len = [inputStream read:buf maxLength:100];
            for (int i=0;i<10;i++)
                NSLog(@"%d:%c", len, buf[i]);
			*/
